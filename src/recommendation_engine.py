...
class ExerciseRepository:
    ...
    async def get_ranked_exercises(
        self, slot_id: int, template_id: int, difficulty_cap: str, equipment: List[str]
    ) -> List[Dict[str, Any]]:
        raise NotImplementedError()
...
class MockExerciseRepository(ExerciseRepository):
    ...
    async def get_template(self, sport: str, role: str, goal: str) -> Optional[Dict[str, Any]]:
        # Hardcoded match for Fast Bowler
        if sport.lower() == "cricket" and "bowl" in role.lower() and goal.lower() == "power":
            return {
                "id": 100,
    ...
    async def get_slots(self, template_id: int) -> List[Dict[str, Any]]:
        return [
            {"id": 201, "slot_type": "Primary", "name": "Max Dynamic Output (Bilateral)", "display_order": 1, "notes": "Bilateral explosive lift designed to increase vertical ground reaction forces."},
            {"id": 202, "slot_type": "Secondary", "name": "Unilateral Force Production", "display_order": 2, "notes": "Unilateral hops or lateral bounds targeting deceleration."},
            {"id": 203, "slot_type": "Accessory", "name": "Triple Extension Acceleration", "display_order": 3, "notes": "Accessory ballistic work utilizing a hip hinge."},
            {"id": 204, "slot_type": "Core", "name": "Trunk Rotational Velocity", "display_order": 4, "notes": "Rotational trunk power to transfer force."}
        ]
    ...
    async def get_ranked_exercises(
        ...
        exercises = [
            ...
            },
            {
                ...
                "required_equipment": ["Medicine Ball"],
                "slots": [204],
                "score_components": {"relevance": 8, "specificity": 9, "transfer": 0.85, "mechanics_bonus": 0.0, "tag_bonus": 4.0}
            },
            {
                "id": 5,
        ...
        for ex in exercises:
            ...
            if slot_id not in ex["slots"]:
                continue
            # Check difficulty cap
            if diff_rank[ex["difficulty_level"]] > cap:
                continue
            ...
            score = (
                ...
                comp["mechanics_bonus"] +            # Mechanics alignment
                comp["tag_bonus"]                    # Tag matching
            )
